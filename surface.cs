...
namespace template
{
    ...
    public class Surface
    {
        ...
        {
            width = w; // to include debug window
            height = h;
            pixels = new int[width * 2 * height];
        }
        ...
        //draw a circle
        public void Circle(Vector3 center, float radius, Screen screen, Vector3 color)
        {
        ...
        public void DrawPrimitives(List<Primitive> primivites, List<Light> lights, Screen screen)
        {
            ...
            {
                Circle(l.Position, 0.1f, screen, l.Color);
            }
        }
        ...
        public void Line(int x1, int y1, int x2, int y2, int c)
        {
            ...
            if (Math.Abs(x2 - x1) >= Math.Abs(y2 - y1))
            {
                ...
                for (int i = 0; i < l; i++)
                {
                    if(x1 < 512)pixels[x1++ + 512 + (y1 / 8192) * width] = c;
                    y1 += dy;
                }
            }
            else
            {
                ...
                for (int i = 0; i < l; i++)
                {
                    if((x1/8192) < 512) pixels[x1 / 8192 + 512 + y1++ * width] = c;
                    x1 += dx;
                }
            }
        }
